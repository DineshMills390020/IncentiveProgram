...
public class IncentiveController {
    ...
    @Autowired
    private WeavingMonthTotalWeavingProductionProductwiseRepository weavingMonthTotalWeavingProductionProductwiseRepository;
    ...
    public String exportIncentiveData(@RequestParam(value = "frmSubmit") String format, HttpServletResponse response, Model model) throws IOException, DocumentException {
        ...
        int totalRound = 0;
        System.out.println("Year "+employeeIncentiveDetailsList.get(0).getIncYear());
        System.out.println("Month "+employeeIncentiveDetailsList.get(0).getIncMonth());
  /*
        List<WeavingMonthTotalWeavingProductionProductwise> allDetailsList = weavingMonthTotalWeavingProductionProductwiseRepository.getAllDetails(employeeIncentiveDetailsList.get(0).getIncMonth(),employeeIncentiveDetailsList.get(0).getIncYear());
        if(allDetailsList.size()>0){
            List<String> colHeaderList1 = Stream.of("SrNo","Prod Group","Dept","PickMtr", "PickKg", "PieceCount", "MthWeight", "Efficiency", "MthPickMtr", "IncRate", "TotalIncAmount").collect(Collectors.toList());
            List<List<String>> excelValues = new ArrayList<>();
            excelValues.add(colHeaderList1);
            int j=1;
            for(WeavingMonthTotalWeavingProductionProductwise w: allDetailsList){
                List<String> valueList = new ArrayList<>();
                valueList.add(String.valueOf(j));
                valueList.add(w.getProductGroup());
                valueList.add(w.getDepartment());
                valueList.add(String.valueOf(w.getPickMtr()));
                valueList.add(String.valueOf(w.getPickKg()));
                valueList.add(String.valueOf(w.getPieceCount()));
                valueList.add(String.valueOf(w.getMthWeight()));
                valueList.add(String.valueOf(w.getEfficiency()));
                valueList.add(String.valueOf(w.getMthPickMtr()));
                valueList.add(String.valueOf(w.getIncRate()));
                valueList.add(String.valueOf(w.getTotalIncentiveAmount()));
                //valueList.add(String.valueOf(w.getPickMonth())+"-"+String.valueOf(w.getPickYear()));
                //valueList.add(String.valueOf(w.getPickMonth()));
                excelValues1.add(valueList);
                j++;
            }
    ...
    public @ResponseBody String calculateMonthlyIncentive1(@RequestParam(value = "yearSel") String yearSel,
        ...
        if (incentiveLock != null && incentiveLock.isIncentiveFlag()) {
         ...
         //   employeeIncentiveDetailsList = employeeIncentiveDetailsRepository.findByMonthAndYear(pickMonth, pickYear);
            ...
            double totalDeptIncentive = weavingMonthTotalWeavingProductionProductwiseRepository.getTotalIncentive(pickMonth,pickYear);
                ...
                for (String empId : empNoList) {
                   //    String empId = "BRD306656";
                    ...
                    if (empVType.equals("true")) {
                        ...
                        System.out.println("Attendance Days" + attendanceDay1);
